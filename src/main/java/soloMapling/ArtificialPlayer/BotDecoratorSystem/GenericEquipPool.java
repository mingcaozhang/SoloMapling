 ...
 * (never pick gear the bot can't wear) and bias toward gear near the bot's
 * level,
...
public class GenericEquipPool {
    ...
    private static final String YAML_PATH = "soloMapling/ArtificialPlayer/BotDecoratorSystem/GenericEquipPool.yaml";

    // Higher = stricter preference for gear near the bot's level. 0.05 gives a
    // gentle tail
    // so a level 95 bot can still occasionally roll a level-35 whip for fashion.
    private static final double LEVEL_DECAY = 0.05;
    // Every eligible item keeps at least this relative weight, so outliers still
    // occur.
    private static final double FASHION_FLOOR = 0.05;

    // Gender constants matching Character.getGender(): 0 = male, 1 = female, 2 =
    // unisex.
    ...
    public static synchronized void load() {
        ...
        // "soloMapling/generic_equips.yaml")
        try (InputStream inputStream = GenericEquipPool.class.getClassLoader().getResourceAsStream(YAML_PATH)) {
            if (inputStream == null) {
            ...
            try (YamlReader reader = new YamlReader(new InputStreamReader(inputStream))) {
                ...
                for (Map.Entry<String, Object> entry : root.entrySet()) {
                    ...
                    // Standard type validation check for older Java versions
    ...
    public static Integer getRandom(String category, int botLevel, int botGender) {
        ...
        for (PoolItem item : list) {
            if (item.minLevel > botLevel)
                continue; // hard rule: never over-level
            if (item.gender != GENDER_UNISEX && item.gender != botGender)
                continue; // gender gate
            if (EquipOmitList.isOmitted(item.id))
                continue; // central omit list (flag/junk items)
